...
export function renderOverview(courses, declaredResults) {
  ...
  auditDegree({ school, courses, programs, loadDetails: (code) => courseDetails(school, code) }).then((d) => {
    ...
    const upperTile = tile('Upper-level hours', `<span class="${d.upper.satisfied ? '' : warn}">${num(d.upper.have)}</span><span class="text-sm font-normal text-zinc-400"> / ${d.upper.need}</span>`, inRes(d.residency?.upper) || '300 level and above');
